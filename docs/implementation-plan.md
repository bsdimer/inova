...
Sosedo is a multi-tenant B2B SaaS platform for professional property-management companies, consisting of (1) a web admin panel for property managers, (2) a white-label resident mobile application, and (3) a shared backend. The pilot launches in Bulgaria under the platform's own **Sosedo** brand (acting as the first tenant). The commercial model bills each tenant monthly **per managed apartment** (the billable "end customer" — a real-estate entity associated with the tenant) for a predefined base feature bundle, at a **per-tenant configurable unit price defaulting to 0.80 EUR**, managed by a platform `super_admin` role through the UI (including bulk price updates across all tenants). Premium add-on features (AI integration, shared document signing for block meetings) carry additional per-apartment fees. Expansion targets are Romania, Serbia, and Poland.
...
- **Architecture (decided by stakeholder):** a **small fixed set of services from day one**: a dedicated **auth service** (issues JWTs carrying tenant-membership claims, verified by other services via JWKS — no runtime call to auth on every request), a **core API** (domain modules behind strict internal boundaries), and a **worker** (queues/cron). This honors the microservices direction where it pays off (authentication as an isolated security domain) while keeping domain modules co-deployed for low hosting cost; further extraction seams (payments, notifications, exports) remain designed-in.
...
- **Payments (decided by stakeholder):** **Stripe Connect** — each tenant onboards a connected Stripe account; resident card payments flow **directly to the tenant's Stripe account** (tenant is merchant of record). The platform never holds resident funds; it consumes Connect webhooks to record, allocate, and reconcile payments. Platform revenue (0.80 EUR/end customer + add-ons) is billed separately via Stripe Billing subscriptions on the platform account.
...
| Question | Finding |
|---|---|
| What exists | Nothing. `/Users/mihailsfolder/dev/sosedo` contains only an initialized `.git` directory with **zero commits**, no branches, no files. |
...
|---|---|---|---|
| B1 | **RESOLVED — Stripe Connect with Standard connected accounts.** Resident payments flow directly to each tenant's connected Stripe account; tenant is merchant of record and owns its Stripe relationship/dashboard; platform only consumes payment webhooks. | — | Decided by stakeholder (including account type). |
| B2 | **Invoicing and fiscal compliance in Bulgaria** — Are the management companies VAT-registered? Must invoice numbering be gapless 10-digit per Bulgarian VAT law? With Stripe Connect the tenant is merchant of record and online card payments via a licensed PSP are generally **exempt from fiscal cash-register receipts under Ordinance N-18**, so this reduces to accountant sign-off of our receipt/invoice PDF templates — but confirm. | M4 (receipt templates), M9 (invoices) | Legal requirement for the documents tenants issue. **Requires a Bulgarian accountant's confirmation — do not guess.** No longer expected to force schema rework. |
| B3 | **RESOLVED — service topology fixed:** dedicated auth service from day one (tenant memberships coded into the JWT), core API as co-deployed domain modules, separate worker. See §4.1. | — | Decided by stakeholder. |
...
|---|---|---|
| Payment provider | **Stripe Connect (decided)** — tenant-owned connected accounts, direct charges; a `PaymentProvider` abstraction is still kept so a local provider (ePay.bg) adapter remains possible for tenants that demand it | Standard account type (decided, B1). Tenant onboarding via Stripe Account Links from the admin panel. |
| Platform billing | **Stripe Billing** on the platform account: per-tenant subscription, monthly metered quantity = **managed apartments** (B6 resolved), unit price **configurable per tenant** via super_admin UI (default 0.80 EUR, bulk-update across all tenants), plus per-apartment add-on prices per premium feature (AI integration, document signing) | Entitlement flags enforced server-side per tenant; metering job snapshots monthly counts (§5.2, M-Bill). Tenants are provisioned through the super_admin console, not by scripts. |
...
| Authentication | **Dedicated auth service (decided)**: email+password (argon2id), email OTP verification, JWT access (10–15 min, RS256/ES256 via JWKS) with **tenant memberships + roles as claims**, rotating refresh tokens; staff SSO later | Avoids Cognito/Auth0 per-MAU cost and keeps EU data residency trivial. Claim staleness bounded by short TTL + revocation denylist (§4.1). |
...
- **A-DEPOSIT:** "Deposit account" = building repair/reserve fund with separate balance from operational cash; transfer rules between them need confirmation.
...
### P0 — required for a usable Sosedo pilot
...
| Sosedo data import tooling (buildings, apartments, residents, opening balances) | Pilot cannot start without it |

### P1 — before broader commercial launch

| Feature | Why deferred from P0 |
|---|---|
| Online card payment via Stripe Connect + webhooks + reconciliation + refunds | Highest-risk integration; pilot works with bank transfer + manual entry. Build immediately after pilot start (M8). |
| Platform billing: Stripe Billing subscriptions, 0.80 EUR/end-customer metering, entitlement flags (M-Bill) | Pilot tenant is the platform's own brand — nothing to invoice yet; must exist before the first paying tenant. Entitlement *flags* land earlier (M1 settings) so features can gate from the start. |
...
| Dedicated white-label build pipeline + first partner's dedicated app | Pilot runs in the shared app under the Sosedo brand (B5 resolved); dedicated apps start when the first white-label partner signs |
...
| Feature | Notes |
|---|---|
| AI integration (premium add-on, per-end-customer fee) | Scope undefined yet; gated behind an `ai_integration` entitlement from day one so pricing plumbing (M-Bill) is ready when the feature lands |
| Shared document signing for block meetings (premium add-on, per-end-customer fee) | Legally sensitive (qualified e-signature rules per country); gated behind a `document_signing` entitlement; pairs naturally with the surveys/protocols module |
...
Stakeholder decision: microservices from day one, with authentication separated at minimum and **tenant memberships coded into the JWT**. To honor this while keeping hosting cost minimal, the day-one topology is a **fixed set of three services** rather than a service-per-module fleet:

1. **`auth-service`** — owns users, credentials, refresh tokens, memberships, and roles. Issues short-lived (10–15 min) access tokens signed with an asymmetric key (RS256/ES256). Token claims: `sub` (user id), `memberships: [{tenant_id, kind: staff|resident, roles[]}]`, optional `platform_role` (`super_admin` for platform operators, §6.2), brand hint. Publishes a **JWKS endpoint**, so all other services verify tokens locally with zero runtime calls to auth — auth being down never blocks already-authenticated traffic.
...
**Handling JWT claim staleness** (the trade-off of tenant-in-token): access-token TTL of 10–15 minutes bounds how long a revoked membership can linger; for immediate revocation (staff dismissal, security incident) auth-service writes the user id to a small Redis denylist that core-api checks on each request (one O(1) lookup, no membership query). Refresh-token rotation re-reads memberships from the database, so every token refresh picks up grant changes.
...
flowchart LR
  ...
  StripeC[Stripe Connect tenant accounts]
...
| Auth | Dedicated auth-service (argon2id, JWT with membership claims + rotating refresh, email OTP, JWKS) | AWS Cognito, Keycloak, Auth0 | No per-MAU fees, EU residency trivial, full control of tenant-membership claims (stakeholder requirement). Cognito is the fallback if the team prefers not to own password storage. |
...
**Mode A — dedicated apps (larger partners).** `app.config.ts` reads `BRAND=sosedo` and produces unique bundle ID (`bg.sosedo.resident`), package name, display name, icons, deep-link associated domains, and Firebase config file per brand. CI builds each brand as a matrix job. Submission happens from the **partner's own** Apple Developer and Google Play accounts, with the platform operator invited in a release-manager role.

**Mode B — shared multi-brand app (smaller partners).** One store listing under the platform's accounts. Tenant selection at first launch via: (1) invitation deep link (`https://app.sosedo.bg/i/<token>`), (2) organization code entry, or (3) automatic association after login when the user's account has exactly one tenant membership. Branding (theme, logo, strings) is applied at runtime from the brand config API and cached locally.

**Security invariant (both modes):** the client's brand/bundle identifies *presentation only*. Every API request is authorized against the authenticated user's tenant memberships server-side. A dedicated app for tenant X presents tenant X's brand, but if a user of that app holds memberships only in tenant Y, they see tenant Y's data or an "no access" state — never tenant X's data.

### 4.4 Tenant isolation design

1. **Authentication layer:** JWT carries the user's tenant memberships and roles as claims (stakeholder decision), signed by auth-service and verified via JWKS. Staleness is bounded by short TTL + revocation denylist + refresh-time re-read (§4.1).
2. **Request context:** every authenticated route requires an explicit `X-Tenant-Id` header (or path param); middleware verifies that tenant id appears in the token's `memberships` claim with an active status and derives the role set from the claim. Absence → 403 before any handler code runs. Highly sensitive operations (payments recording, role changes, refunds, GDPR actions) additionally re-verify the membership against the database, not just the token.
...
4. **Object storage:** S3 keys are prefixed `tenants/<tenant_id>/…`; presigned URLs are minted only after the same membership check.
5. **Cross-tenant users:** a resident may hold memberships in multiple tenants (e.g., apartments managed by different companies). The mobile app shows a tenant/property switcher; switching changes the `X-Tenant-Id` header, never merges data.
...
`users` are global (one login per person). `occupancies` link a user to an apartment with a role (`owner`, `tenant`, `occupant`, `proxy`) and validity dates. A user can hold many occupancies across buildings and tenants. The mobile home screen aggregates obligations per apartment with an apartment switcher; notifications carry `(tenant_id, apartment_id)` context.
...
|---|---|---|
| `tenant` | Management company; holds locale/currency/timezone/settings JSON; **Stripe Connect fields**: `stripe_account_id`, onboarding status (`none → pending → active → restricted`), payouts-enabled flag | `trial → active → suspended → offboarded` |
| `subscription` | Platform billing per tenant (Stripe Billing subscription id, **per-tenant unit price** — default 0.80 EUR per managed apartment, super_admin-configurable with bulk update, price history kept for invoice auditability, currency, status) | `trialing → active → past_due → canceled` |
| `entitlement` | Feature grants per tenant: base bundle + add-ons (`ai_integration`, `document_signing`, …) with per-apartment add-on price refs; enforced server-side by a feature guard; managed via super_admin console | `active → suspended → removed` |
| `usage_snapshot` | Immutable monthly metering record per tenant: **count of managed apartments** (B6 resolved), per-entitlement counts, unit prices in effect, computed at period close; reported to Stripe Billing as metered usage | — |
| `brand` | 1..n per tenant; distribution mode (`dedicated`/`shared`); config JSON; store identifiers | `draft → live → retired` |
| `user` | Global identity; email (unique), phone, password hash, locale | `pending_verification → active → disabled → erased(GDPR)` |
| `staff_membership` | `(user, tenant, role_set)`; invitation-based | `invited → active → suspended → revoked` |
| `role` / permissions | Per-tenant custom roles composed of fixed permission keys (e.g. `billing.write`, `issues.manage`) | — |
| `building` / `entrance` / `apartment` | Apartment carries area m², property type, ideal parts, occupant count | apartment: `active → archived` |
| `occupancy` | `(user, apartment, role, valid_from/valid_to)`; admin-verified | `requested → verified → active → ended → rejected` |
| `fee_rule` | Basis: `fixed / per_area / per_occupant / per_ideal_part`; scope building/entrance/apartment-type; versioned (new version, never edit in place) | `active → superseded → archived` |
...
| `payment` | Immutable; source `manual / stripe_connect`; method; connected-account + PaymentIntent refs; idempotency key | `pending → confirmed → failed → refunded` (provider); manual are `confirmed` at entry, corrected only by reversal |
| `payment_allocation` | Immutable `(payment, charge, amount)` split | — |
| `cash_account` | Per building: `operational` and `deposit` kinds; balance **derived** from ledger | — |
...
| `expense` | Building cost, optional contractor, optional recurrence template | `draft → approved → paid → reversed` |
| `receipt` / `invoice` / `credit_note` | Numbered documents; gapless per-tenant series; rendered PDF stored in S3; invoice corrections only via credit note | invoice: `issued → corrected → void(by credit note)` |
...
| `survey` / `ballot` / `vote` | Ballot = one voting right per apartment (or per ideal part — confirm with Sosedo, assumption A-VOTE); vote immutable once cast | survey: `draft → open → closed → protocol_issued` |
...
| Cash-account balance | **Calculated** from `ledger_entry` sum; periodic snapshot rows for fast paging and audit checkpoints | Ledger is truth |
...
- **Residents (mobile):** email+password with email OTP verification; refresh-token rotation with reuse detection (token family revocation); device records for push tokens.
...
- **Goal:** monthly fees generate correctly; ad-hoc charges; residents' obligation math is right.
- **Dependencies:** M2.
- **DB:** `fee_rules (versioned), charges, charge_lines`; period fields; `document_counters`.
- **Backend:** fee-rule CRUD with versioning; **fee-generation job** (BullMQ cron, per tenant timezone, idempotent per `(rule_version, apartment, period)` unique index); one-time/temporary charges; obligations query views.
- **Admin:** fee-rule builder (basis: fixed/per-m²/per-occupant/per-ideal-part; scope), charge list/detail, manual charge entry, generation preview ("dry run" diff before commit).
- **Mobile:** obligations screen (current + history per apartment), IBAN + payment reference display with copy actions.
- **Tests:** golden-file financial tests (rule × apartment matrix → expected charges), no-proration boundary cases (mid-month basis change applies next period, per A-FEE), idempotent re-run of generation, timezone boundary (month end in Europe/Sofia).
- **Acceptance:** Sosedo's actual fee schedule reproduced to the stotinka against a hand-calculated sheet.
- **Risks:** proration and allocation rules are resolved (A-FEE/A-ALLOC); remaining workshop item before M4 is deposit-account transfer rules (A-DEPOSIT).

#### M4 — Manual payments, cash accounts, receipts (L)
- **Goal:** managers record bank/cash payments, allocate across charges; balances and receipts correct.
- **Dependencies:** M3.
- **DB:** `payments, payment_allocations, cash_accounts, ledger_entries, receipts`; append-only triggers; idempotency table.
- **Backend:** payment entry with auto-allocation (oldest-first default, manual override), reversal flow, ledger writer, receipt PDF job, cash/deposit account summaries + transaction history.
- **Admin:** payment entry screen (fast keyboard-driven), allocation editor, building cash/deposit dashboards, receipt viewing/printing.
- **Mobile:** payment history + receipt download; building cash-account summary (read-only aggregates).
- **Tests:** allocation invariants (Σ allocations = payment amount; never over-allocate a charge), reversal round-trip, ledger balance property tests, gapless receipt numbering under concurrency.
...
- **Acceptance:** internal TestFlight/Play internal-testing build of the **shared app** used by the team with real Sosedo staging data.
...
#### M-Pilot — Sosedo onboarding and launch (M)
- **Goal:** Sosedo live in production on the shared app.
...
#### M8 — Online payments via Stripe Connect (L, starts parallel to pilot, ships as fast-follow)
- **Goal:** resident pays by card in-app; funds land **directly in the tenant's connected Stripe account** (tenant is merchant of record); platform records, allocates, and reconciles from webhooks.
- **Dependencies:** M4. Account type decided: Standard (B1).
- **DB:** `payment_intents, provider_events (unique provider_event_id), refunds`; `stripe_account_id` + onboarding status on tenant.
- **Backend:** tenant Stripe onboarding flow (Account Links from admin panel, `account.updated` webhook tracking onboarding/`payouts_enabled` status); `PaymentProvider` port + Stripe Connect adapter — PaymentIntents created as **direct charges on the connected account** (`Stripe-Account` header), initiation with `Idempotency-Key`; **Connect webhook endpoint** (account-scoped events, signature-verified) → confirm payment → auto-allocate → receipt → push; refund flow executed on the connected account (permission-gated, audit-logged); per-connected-account reconciliation job (§8.2); provider-outage degradation (bank-transfer instructions always available); pay-online CTA hidden until the tenant's account is `active`.
- **Admin:** Stripe onboarding status + "complete onboarding" link, provider payment visibility, refund action, reconciliation exceptions screen.
- **Mobile:** pay-now flow (Payment Sheet against the connected account), 3DS handling, result states, receipt.
- **Tests:** Connect webhook replay/duplicate/out-of-order tests; idempotency tests; refund lifecycle; onboarding state machine; provider sandbox e2e with two connected test accounts (isolation: tenant A's webhook can never mutate tenant B's payment). **Release blocker tier.**
- **Risks:** tenant KYC friction during Stripe onboarding (mitigate with guided checklist + status visibility); Bulgarian availability of required capabilities per account type — verify in Stripe test mode early.

#### M-Bill — Platform billing and entitlements (M, after M2; required before commercial launch)
- **Goal:** the platform invoices each tenant per **managed apartment** per month (per-tenant unit price, default 0.80 EUR), plus per-apartment add-on fees for premium features (AI integration, document signing); features gate on entitlements; all administered by super_admin through the UI.
...
- **Acceptance:** a test tenant with a known apartment count produces the exactly expected Stripe invoice amount for base + one add-on; a bulk price change is reflected in the next period's snapshots only.
- **Risks:** apartment-count edge cases (archived vs. active mid-month — bill on period-close count, flagged as assumption A-METER) need commercial confirmation.
...
- **Work:** brand build matrix in CI (EAS Build or fastlane lanes per brand), credentials store integration (§WL below), per-brand Firebase project + APNs key wiring, store metadata pipeline (fastlane deliver/supply), deep-link domain per brand, differentiation record + compliance checklist execution (§WL), partner account-access procedure executed with Sosedo; shared-app invitation/org-code tenant selection (P1 feature).
- **Acceptance:** Sosedo app approved on both stores; a second test brand builds from config alone with zero code changes.
...
Invoices & credit notes (M, needs B2), recurring expenses + contractors (M), surveys/voting/protocols (L, needs condo-law review), privileges module (M), notification campaigns UI (S), full report/export catalog (M), shared-app multi-tenant selection polish (S), dual BGN/EUR display (S), audit-trail screen (S), four-eyes reversals (S).
...
| `property` | buildings, entrances, apartments, bulk import, occupancy requests/verification, pets |
| `billing` | fee rules (+versions, dry-run preview), charges, obligations views |
| `payments` | manual payments, allocations, reversals, payment intents (initiate), refunds, provider webhook (`POST /v1/webhooks/payments/:provider` — unauthenticated, signature-verified) |
| `finance` | cash accounts, ledger, deposits, expenses, contractors |
| `documents` | receipts, invoices, credit notes, PDF download links |
...
| `surveys` | surveys, ballots, votes, protocol |
| `reports` | dashboard aggregates, debtors, export jobs |
| `brands` | `GET /v1/brands/:key/config` (public, non-secret runtime branding) |

### 8.2 Payment flow contracts (Stripe Connect)

- **Model:** each tenant holds a **Standard connected Stripe account** (decided); resident charges are **direct charges on the connected account** — funds settle to the tenant, the platform never holds resident money and only records what happened. Platform fees are *not* taken per transaction; platform revenue is the separate Stripe Billing subscription (§M-Bill).
- **Tenant onboarding:** admin panel → Account Link (hosted Stripe onboarding) → `account.updated` webhooks update `stripe_account_id` status; online payments enabled only when `payouts_enabled`.
- **Initiation:** `POST /v1/payment-intents {apartment_id, charge_ids?, amount}` with `Idempotency-Key` → PaymentIntent created on the tenant's connected account (`Stripe-Account` header) → client secret to the app. Server computes payable amount; client amount is advisory only. Metadata carries `{tenant_id, apartment_id, intent_id}` for correlation.
- **Webhooks:** single Connect webhook endpoint receiving account-scoped events; verify signature → resolve tenant by the event's `account` field (**server-side lookup — never trust client/tenant headers here**) → insert into `provider_events` (unique `provider_event_id`, duplicates return 200 and stop) → enqueue processing job → job confirms payment, allocates, issues receipt, sends push. Handler is idempotent and tolerant of out-of-order events (state machine checks).
- **Reconciliation:** nightly job per connected account pulls the charge/balance-transaction list for the period and diffs against local `payments`; mismatches create reconciliation-exception records surfaced in admin; no auto-mutation of ledger.
- **Refunds:** admin-initiated, permission-gated → refund API on the connected account → refund webhook closes the loop with reversing ledger entries; partial refunds supported.
- **Duplicate-event protection:** provider-event unique index + intent state machine + allocation invariants (a charge can never be allocated beyond its amount) as defense in depth.
- **Platform billing flow (§M-Bill):** monthly metering job → `usage_snapshot` → metered usage reported to Stripe Billing on the **platform account** → invoice webhooks drive tenant dunning state.
...
| Platform-billing metering | Monthly, period close | Counts billable end customers per tenant (B6), writes `usage_snapshot`, reports to Stripe Billing |
...
| Financial calculation | Golden-file suites: fee matrix, allocations, ledger balancing, reconciliation; hand-verified Sosedo oracle sheet | **Release blocker** |
...
- [ ] Incident-response contact tree agreed with Sosedo
...
Feedback collection: in-app support contact, monthly Sosedo review call, PostHog funnels on activation/payment flows, app-store review monitoring.
...
|---|---|---|
| For | Larger white-label partners | Smaller partners (and the platform's own Sosedo brand) |
...
| Tenant selection | Brand → default tenant, but **authorization always via membership** (§4.4) | Invitation deep link, organization code, or single-membership auto-select |
...
**Contingency for blocked/expired accounts:** if a partner account lapses or is blocked, the dedicated app may be removed from sale but installed apps keep working (backend unaffected). Recovery ladder: (1) restore partner account; (2) temporary migration of residents to the shared multi-brand app (same login works — tenant membership is server-side, so this is a store-listing swap, not a data migration); (3) if permanent, republish under a resolved account. Communication templates for residents prepared in advance. This is a key argument for building Mode B regardless of Mode A demand.

### WL.4 Backend authorization invariant

Restated as a hard rule: bundle ID, package name, brand key, or any client-supplied configuration is **never** an authorization input. Tenant access derives exclusively from authenticated membership rows checked server-side per request, with RLS as backstop. Dedicated apps merely *default* the presentation brand.
...
```
sosedo/  (monorepo: pnpm workspaces + Turborepo)
├── apps/
...
├── brands/
│   └── sosedo/{brand.json, assets/, store/}   # non-secret brand config
├── db/
...
| R5 | Online card/bank payment via Stripe Connect (funds direct to tenant) | P1 | M8 |
...
| X2 | Sosedo pilot onboarding/migration | P0 | M-Pilot |
| X3 | Platform billing: per-apartment subscriptions (per-tenant price, default 0.80 EUR, super_admin console + bulk update) + entitlements | P1 | M-Bill |
| X4 | Premium add-ons: AI integration, shared document signing (per-end-customer fees) | P2 (features) / P1 (entitlement gating) | M-Bill + P2 |
| X5 | Separate auth service, tenant memberships in JWT | P0 | M0–M1, §4.1 |
...
| Tenant Stripe Connect onboarding friction (KYC, BG capability gaps) | Medium | Medium | Pilot works without online payments; guided onboarding checklist + status tracking in admin; verify BG capabilities in Stripe test mode early | Product owner |
| Stale JWT membership claims grant access after revocation | Low | High | 10–15 min token TTL, revocation denylist checked per request, DB re-check on sensitive operations, refresh re-reads memberships | Backend lead |
...
| Opening-balance disputes at migration | Medium | High | Dry-run + signed reconciliation report before go-live | Backend lead + Sosedo |
| Tenant data leak (isolation bug) | Low | Critical | RLS + membership middleware + blocker test suite in CI; pen test before commercial launch | Backend lead |
| Fee-calculation errors erode trust | Medium | High | Golden-file oracle from Sosedo's real data; dry-run preview before each generation | Backend lead |
...
|---|---|---|---|
| D1 | Service topology | **Decided:** auth-service + core-api + worker; tenant memberships in JWT; further extraction seams designed-in (§4.1) | Resolved by stakeholder |
| D2 | Payment provider + merchant of record | **Decided:** Stripe Connect, direct charges to tenant-owned connected accounts; platform consumes webhooks only | Resolved by stakeholder |
| D2a | Connect account type | **Decided: Standard** — tenant owns its Stripe account and dashboard; lowest platform liability | Resolved by stakeholder |
| D2b | Billable unit for platform fee (B6) | **Decided:** per managed apartment (real-estate entity) associated with a tenant; unit price configurable per tenant + bulk update via super_admin UI; default 0.80 EUR | Resolved by stakeholder |
| D3 | Receipt/invoice legal shape (gapless, VAT) | In-app PDF documents, gapless numbering; no fiscal device. With Connect, card payments via PSP are typically N-18-exempt — accountant signs off templates only | Reduced scope (B2) — accountant confirmation |
| D4 | Pilot distribution | **Decided:** pilot in the shared app under the platform's own Sosedo brand; dedicated apps at M10 when first white-label partner signs | Resolved |
...
| D7 | Fee proration + allocation order rules | **Decided:** no proration (generation-day rule applies to whole month); oldest-first allocation with per-payment override. Still open from the workshop: deposit-account transfer rules (A-DEPOSIT) | Partially resolved; deposit rules before M4 |
| D8 | Survey voting weight (per apartment vs. ideal parts) | Per ideal parts per BG condo law | Needs legal/Sosedo confirmation before P1 surveys |
...
18. [ ] Property CRUD APIs + occupancy request/verify/reject + resident occupancy guards
...
23. [ ] Fee-rule CRUD + versioning + dry-run preview API
...
Payments & finance
28. [ ] Migration: payments, payment_allocations, cash_accounts, ledger_entries, receipts, idempotency_keys (+append-only triggers)
29. [ ] Manual payment entry + auto/manual allocation + reversal flow (+property tests)
30. [ ] Ledger writer + cash/deposit account summaries + transaction history APIs
31. [ ] Gapless receipt numbering + PDF render job (Playwright in worker) + S3 storage
...
Pilot
47. [ ] Sosedo import scripts (B4 source) with dry-run + opening-balance reconciliation report
...
51. [ ] Stripe Connect: tenant onboarding (Account Links + `account.updated` tracking), direct-charge intents, Connect webhook consumer (signature + account-scoped tenant resolution + dedupe), auto-allocation, refunds, per-account reconciliation job + exceptions UI
51a. [ ] Platform billing (M-Bill): subscriptions (price history)/entitlements/usage_snapshots migrations, monthly apartment-count metering job (rule behind one interface, A-METER), Stripe Billing metered reporting, invoice webhooks + dunning, super_admin screens: per-tenant price config, bulk price update, entitlement toggles, usage history
...
53. [ ] Expenses + contractors module; recurring expense job
...
56. [ ] Shared-app tenant selection (invite deep link, org code)
57. [ ] Surveys/voting/protocols module (after D8 legal confirmation)
...
Resolved by stakeholder: service topology (auth-service + core-api + worker, tenant memberships in JWT), Stripe Connect with tenant-owned accounts, pilot under the platform's own Sosedo brand in the shared app, and the commercial model — per managed apartment, per-tenant unit price (default 0.80 EUR) configured through the super_admin console with bulk update, plus premium add-ons.

Also resolved since: Stripe Connect account type (**Standard**), payment allocation (**oldest-first with per-payment override**), fee proration (**none — generation-day rule applies to the whole month**), platform metering (**period-close apartment count**), and pilot data source (**Excel/Google Sheets** — spreadsheet importers).

Still open:

1. **Bulgarian fiscal/invoicing compliance (D3/B2):** accountant sign-off on receipt/invoice PDF templates and gapless numbering. With Connect, card payments via a licensed PSP are typically exempt from fiscal cash-register receipts under Ordinance N-18, but this must be confirmed, not assumed.
2. **Deposit-account transfer rules (A-DEPOSIT/D7):** how money moves between a building's operational and deposit accounts (who authorizes, whether transfers are restricted) — needed before M4.
3. **Survey voting weight (D8):** per apartment vs. per ideal parts under Bulgarian condominium law — needed before the P1 surveys/voting module (and it gates the premium document-signing add-on that pairs with meeting protocols).
