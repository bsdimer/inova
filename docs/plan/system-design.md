...
2. **`core-api`** — the domain modules (`tenancy`, `property`, `billing`, `payments`, `finance`, `documents`, `files`, `issues`, `notices`, `notifications`, `tasks`, `privileges`, `surveys`, `reports`, `platform-billing`) behind strict internal boundaries. `files` owns the shared attachment infrastructure and the uploaded-document library; `documents` stays reserved for numbered fiscal documents (receipts, invoices, credit notes).
