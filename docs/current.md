...
- Test environment: `https://portal.whitenova.tech` (204.168.180.167) runs the
  `infra/deploy/` compose stack — admin SPA at `/`, core-api at `/api/v1`,
  auth-service at `/auth/v1`, Let's Encrypt TLS renewed by a systemd timer.
  `.github/workflows/deploy.yml` builds amd64 images to GHCR and deploys over
  SSH after a green CI run on `main`. Host secrets are generated on the box and
  live only there.
...
6. Test-environment hygiene — the demo seed ran on the public host, so rotate or
   drop those credentials, and add a nightly `pg_dump` off the box.
