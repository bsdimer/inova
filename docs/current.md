...
| M1 Identity                          | Original global-user backend + admin screens implemented; B8 refactor required before M2                                                                                                                             | Tenant-scoped accounts (same email/phone allowed independently per tenant), one-tenant-per-token authorization, multi-role context, separate platform identities; then Redis denylist, worker, real delivery, silent refresh, audit viewer.   |
