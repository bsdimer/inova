...
Archived from the former `docs/implementation-status.md`. Newest first.
Each new session adds a short entry: what changed, how it was verified, what remains.

### 2026-08-31 — Portable frontend harness and workflow

- Kept `.cursor/skills/inova-frontend/` as the canonical project workflow and
  added a thin Claude adapter plus `CLAUDE.md` importing `AGENTS.md`; Codex reads
  the same canonical path, so no project `.agents/` copy is needed.
- Added small/feature/domain/bug routes, an optional feature brief, risk-based
  TDD guidance, and a concise inova clean-code profile derived from
  `HelgaZhizhka/mentor-resources`. Personal helper skills are optional; the
  repository workflow remains self-contained when they are unavailable.
- Added `pnpm check:admin` and an executable `check:agent-harness` gate in local
  verify and CI. The gate also keeps the Claude adapter auto-invocable while the
  canonical Cursor skill remains explicit-only. Ignored Claude's session-local
  settings file and kept only its shareable project adapter.
- Verification: a Claude CLI smoke test auto-invoked the adapter and loaded the
  canonical routes; `pnpm check:agent-harness`, `pnpm check:admin`, and full
  `pnpm verify` pass with local Postgres.
  Remaining: evaluate the workflow on the first real M2 frontend feature and
  adjust only if it creates friction.

### 2026-08-30 — CI: pnpm/action-setup version clash

- `pnpm/action-setup@v4` now fails when both the workflow `version` input and
  `package.json` `packageManager` are set. Removed `version: 10` from all six
  CI jobs so the action uses `pnpm@10.34.5` from `package.json`. Bumped
  `actions/checkout` and `actions/setup-node` to v5 (Node 24 action runtime).
  The Node 20 deprecation line on `pnpm/action-setup@v4` is a warning from
  that action still declaring `runs.using: node20`; GitHub already executes
  it on Node 24. Do not set `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION`.
- Verified: `pnpm format:check`, lint, typecheck, unit. `pnpm verify`
  integration step needs local Postgres; Docker was not running here.
  Remaining: push to `main` / open a PR so GitHub Actions jobs actually run.

### 2026-08-30 — Agent/dev harness (docs split, verify/doctor, contracts)

- Split living status out of `implementation-status.md` into `docs/current.md`
  (what works, honest milestone gaps, Next up) and this monthly work-log.
  Milestone acceptance lives under `docs/milestones/`. `AGENTS.md` is now
  stable rules + session ritual only. Shareable Cursor rule
  `.cursor/rules/inova-harness.mdc` and frontend skill
  `.cursor/skills/inova-frontend/`.
- Executable DoD: `pnpm verify` (format:check, lint, typecheck, unit,
  integration, architecture checks, build) and `pnpm doctor`.
- Real ESLint in every workspace (hooks, unused imports, `any`, no
  cross-module schema imports, no raw `console` in production). `format:check`
  is a CI gate. Tenant schema + route-access contracts; migration runner
  advisory lock + checksum. CI split into required jobs.
- Review hardening: route classification now parses each handler and requires
  the real JWT/tenant/permission guard chain; the tenant-index contract examines
  every index with explicit identity-scope exceptions; migrations and their
  ledger rows are always committed atomically by the runner.
- Verified: `pnpm verify`. Remaining harness work: Testcontainers, generated
  mobile tokens from `brand.json`, OpenAPI/client drift (see current.md).
...
- Welcome screen backdrop replaced with stakeholder-provided artwork that has the
  ...
  source of truth copied to `brands/inova/assets/welcome-hero.jpg` and referenced
...
- Follow-up 2 (stakeholder review): lockup switched to the client's **"inova — by
  WhiteNova Technology"** mark (gold rule + dot); Сграда tab uses the fully blurred
...
- Feature-parity pass against a competitor resident app (LIVO): unit feed, building
  ...
  language + theme switches — all in the warm inova design, **all on mock data**
...
- `brands/inova/brand.json` + `apps/mobile/src/theme/tokens.ts` rewritten (palette,
...
- `pnpm build`, `pnpm typecheck`, `pnpm --filter @inova/api test` all green; flows
...
- Verified on iPhone 16 Pro Max simulator; `@inova/mobile` typecheck green.
...
- RLS: tenant-owned tables use `app.tenant_id` (via `NULLIF(current_setting(...),'')` —
  ...
  (policy-based, never a role-level bypass). App role `inova_app` (no BYPASSRLS).
...
- Drizzle + pg as `inova_app`; all queries in `identityTx` (SET LOCAL identity scope).
...
- Each suite provisions its own database (`inova_test_api` / `inova_test_auth`) from
...
- `@inova/mobile` typecheck green; verified in iOS Simulator.
...
- Replaced `brands/inova/assets/brand-concept.jpg` with the higher-quality original
...
- `brands/inova/brand.json`: palette from brand concept (navy `#0F1D3A`, blue `#356DFF`,
  green `#22B88F`, purple `#7A6CFF`, mist `#EEF2F7`), gradients, light/dark theme tokens,
  tagline, bundle IDs (`bg.inova.resident`), feature flags.
- Brand concept image at `brands/inova/assets/brand-concept.jpg`.
...
- Components: `GradientButton` (slide-up + press-scale + haptics), `PressableScale`,
  `InovaMark`/`InovaWordmark` (gradient ring logo, green E), `TextField` (animated focus ring).
...
- Port 4000 is occupied by an unrelated process on the dev machine — use
  `API_PORT=4100 pnpm --filter @inova/api dev` locally if needed.
...
- `docs/implementation-plan.md` v1.0 written and iterated with stakeholder decisions:
  auth-service split + tenant-in-JWT, Stripe Connect (Standard), inova pilot brand,
...
- Repo pushed to `git@github.com:bsdimer/inova.git`.
